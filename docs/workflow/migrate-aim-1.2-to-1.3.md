...
# Migrate AIM 1.2 to AIM 1.3

Use this when a repository already follows AIM 1.2 and needs the AIM 1.3 runtime contract.

## Copy-paste prompt

```text
Upgrade this repository from AIM 1.2 to AIM 1.3.
...
6) Startup, resume, and validator behavior remain consistent with AIM 1.3 runtime docs.
7) No stale AIM 1.2-only runtime wording remains where AIM 1.3 behavior is expected.

Actions:
1) Update repository docs and agent guidance to AIM 1.3 runtime terminology.
...
- update workflow docs so startup, resume, validation, and ownership rules match AIM 1.3
...
- removed or replaced:
  - legacy files that try to own current gate, role, or acceptance state outside the official AIM 1.3 checkpoint
